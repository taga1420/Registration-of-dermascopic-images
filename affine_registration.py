...
class affine_registration(object):
  def __init__(self, X, Y, B=None, t=None, sigma2=None, maxIterations=150, tolerance=0.001, w=0, escala=1):
  ...
  def my_transformPointCloud(self, Y):
    return np.dot(Y, np.transpose(self.B)) + np.tile((1/self.escala)*np.transpose(self.t), (Y.shape[0], 1))
